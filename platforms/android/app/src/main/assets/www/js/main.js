...
function loadDirections() {
    ...
    $.ajax(
          {
              type: "GET",
              url: "https://www.rtd-denver.com/api/nextride/routes/" + $("#routeSelect").val(),
    ...
    function processDirections(n)
    {
        ...
        var directions = n.data.attributes.direction;
        $(list).empty();
        $(list).append($("<option disabled/>").val("0").text("- Select Direction -"));
        for(var i=0; i<directions.length; i++)
        {
            var tripHead = directions[i];
            $(list).append($("<option />").val(tripHead.directionId).text(tripHead.directionName));
        }
...
function loadStops() {
    ...
    $.ajax(
          {
              type: "GET",
              url: "https://www.rtd-denver.com/api/nextride/routes/" + $("#routeSelect").val() + "/stops",
              //contentType: "application/json;	charset=utf-8",
              dataType: "json",
              success: function (msg) {
                processStops(msg);
...
function processStops(t)
{
    var stopList = $("#routeStopSelect");
    $(stopList).empty();
    $(stopList).append($("<option disabled/>").val("0").text("- Select Stop -"));
    var directionStops = t.data.filter(function(item){
        return item.attributes.direction[0].directionId === parseInt($("#routeDirectionSelect").val());
    });

    for(i=0; i<directionStops.length; i++)
    {
        $(stopList).append($("<option />").val(directionStops[i].id).text(directionStops[i].attributes.stopName));
    }
...
var outputContainer = $('.js-next-bus-results');
    ...
    $.ajax(
          {
              type: "GET",
              url: "https://www.rtd-denver.com/api/nextride/stops/" + $("#routeStopSelect").val(),
              //contentType: "application/json;	charset=utf-8",
              dataType: "json",
